...
function App() {
  ...
  const handleInputChange = (e, isUserCurrency ) => {
    ...
    if (isUserCurrency) {
      ...
      e.target.value==="" ? setForeignCurrency("") : setForeignCurrency(currency(e.target.value).multiply(exchangeRate.value));
    } else { 
      setForeignCurrency(e.target.value);
      e.target.value==="" ? setUserCurrency("") : setUserCurrency(currency(e.target.value).divide(exchangeRate.value));
    }
  }
